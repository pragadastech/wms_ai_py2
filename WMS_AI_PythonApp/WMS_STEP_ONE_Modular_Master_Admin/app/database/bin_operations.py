...
async def store_bin_counts(bin_data: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Any]:
    ...
    Args:
        ...
        {
            "98789": [
                {"itemId": 71190, "quantity": 2, "locationId": 9},
                {"itemId": 62063, "quantity": 1, "locationId": 9}
            ]
        }
...
async def fetch_bin_inventory() -> Dict[str, Any]:
    """
    Fetch bin inventory data from the bin_count_records table
    
    Returns:
        Dictionary with operation results and bin inventory data
    """
    try:
        # Fetch records from bin_count_records table
        response = supabase.table("bin_count_records").select("*").eq("is_printed", False).execute()
        
        if not response.data:
            logger.info("No unprocessed bin records found in the database")
            return {
                "message": "No bin inventory data found",
                "total_records": 0,
                "data": []
            }
        ...
        return {
            "message": "Bin inventory data fetched successfully",
            "total_records": len(response.data),
            "data": [record["bin_data"] for record in response.data if "bin_data" in record]
        }
    
    except Exception as e:
        logger.error(f"Error fetching bin inventory data: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error fetching bin inventory data: {str(e)}") 