...
from app.database.bin_operations import fetch_bin_inventory
...
async def process_new_records_background():
    ...
    while True:
        try:
            ...
            try:
                query = supabase.table("bin_count_records").select("*").eq("is_printed", False)
                    ...
                    for record in response.data:
                        try:
                            ...
                            update_data = {
                                ...
                                "is_printed": result.get("status") == "success"
                            }
...
@router.post("/netsuite/bin-inventory", 
            summary="Push All Bin Inventory Records to NetSuite",
            description="""
            Retrieve all bin inventory data from the database and push each record individually to NetSuite.
            
            This endpoint:
            1. Fetches all bin inventory data from the local database
            2. Sends each bin record individually to NetSuite
            3. Collects and returns acknowledgments for each record
            
            This approach provides better control and error tracking for each individual bin record.
            
            This endpoint requires authentication with a valid JWT token.
            """,
            response_description="NetSuite acknowledgments and operation results",
            response_model=Dict[str, Any])
async def push_bin_inventory_to_netsuite(_: str = Depends(verify_token)):
    """
    Push bin count records to NetSuite
    """
    try:
        # Fetch bin count records from database
        bin_inventory_result = await fetch_bin_inventory()
        
        if bin_inventory_result["total_records"] == 0:
            return {
                "status": "success",
                "message": "No bin count records to push to NetSuite",
                "data": []
            }
        
        # Process each bin count record
        results = []
        for bin_data in bin_inventory_result["data"]:
            try:
                # Log the bin data for debugging
                logger.info(f"Processing bin data: {bin_data}")
                ...
                results.append({
                    "bin_id": str(bin_data.get("binId", "unknown")),
                    "status": "success",
                    "acknowledgment": result
                })
                
                # Update the record status in the database
                if "binId" in bin_data:
                    update_response = supabase.table("bin_count_records").update({
                        "is_printed": True
                    }).eq("bin_id", str(bin_data["binId"])).execute()
                    logger.info(f"Updated record status for bin_id: {bin_data['binId']}")
                
            except Exception as e:
                # Record the failure but continue processing other bins
                logger.error(f"Error processing bin data: {str(e)}")
                results.append({
                    "bin_id": str(bin_data.get("binId", "unknown")),
                    "status": "failed",
                    "error": str(e)
        ...
        return {
            "status": "success",
            "message": "Bin inventory processing complete",
            "total_records": len(results),
            "results": results
        }
    except Exception as e:
        # Handle and return any errors
        logger.error(f"Failed to push bin inventory to NetSuite: {str(e)}")
        return {
            "status": "error",
            "message": "Failed to push bin inventory to NetSuite",
            "error": str(e)
        }
...
@router.post("/netsuite/process-all-records", 
            summary="Process All Bin Count Records",
            description="""
            Process all records from the bin_count_records table and send them individually to NetSuite.
            
            This endpoint:
            1. Fetches all unprocessed records from bin_count_records table
            2. Processes each record individually using send_single_bin_to_netsuite
            3. Updates the record status in the database upon successful NetSuite integration
            4. Stores the NetSuite response in the database
            5. Returns detailed results for each processed record
            
            This endpoint requires authentication with a valid JWT token.
            """,
            response_description="Processing results for all bin count records",
            response_model=Dict[str, Any])
async def process_all_bin_records(_: str = Depends(verify_token)):
    """
    Process all bin count records and send them to NetSuite individually
    """
    return await process_new_bin_records()
